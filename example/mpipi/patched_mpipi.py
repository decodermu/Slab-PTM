...
def simulate(all_parms,name,prot,temp,cutoff):
    ...
    energy_expression = 'eps*alpha*((s/r)^(2*mu)-1)*((3*s/r)^(2*mu)-1)^2'
    energy_expression += '; alpha=2*3^(2*mu) * (3/2/(3^(2*mu)-1))^3'
    energy_expression += '; eps=eps_unit*epsTable(type1,type2); s=s_unit*sTable(type1,type2); mu=mu_unit*muTable(type1,type2)'
    wf = openmm.openmm.CustomNonbondedForce(energy_expression)
    ...
    wf.addGlobalParameter('eps_unit',1*unit.kilojoules_per_mole)
    wf.addGlobalParameter('s_unit',1*unit.nanometer)
    wf.addGlobalParameter('mu_unit',1*unit.dimensionless)
    wf.addPerParticleParameter('type')

    ntypes = 25
    
    epsTable = all_parms['eps'].to_numpy().flatten()
    epsTable = openmm.openmm.Discrete2DFunction(ntypes, ntypes, epsTable)
    wf.addTabulatedFunction("epsTable", epsTable)
    
    sTable = all_parms['s'].to_numpy().flatten()
    sTable = openmm.openmm.Discrete2DFunction(ntypes, ntypes, sTable)
    wf.addTabulatedFunction("sTable", sTable)
    
    muTable = all_parms['mu'].to_numpy().flatten()
    muTable = openmm.openmm.Discrete2DFunction(ntypes, ntypes, muTable)
    wf.addTabulatedFunction("muTable", muTable)
    ...
    wf.setNonbondedMethod(openmm.openmm.CustomNonbondedForce.CutoffPeriodic)
    hb.setUsesPeriodicBoundaryConditions(True)
    yu.setCutoffDistance(4*unit.nanometer)
    #ah.setCutoffDistance(cutoff*unit.nanometer)
    wf.setCutoffDistance(cutoff*unit.nanometer)
    ...
    print(wf.usesPeriodicBoundaryConditions())
    ...
    simulation.reporters.append(app.statedatareporter.StateDataReporter('{:s}_{:d}.log'.format(name,temp),10000,
...
if __name__ == "__main__":
    ...
    parser.add_argument('--parm_file', type=str, default="mpipi_ptm_parms.xlsx", help="Forcefield parameter file.")
    ...
    sheets_config = {
        "eps": {"header": 0, "index_col": 0},
        "s": {"header": 0, "index_col": 0},
        "mu": {"header": 0, "index_col": 0},
    ...
    proteins = pd.DataFrame(
        [[args.pH, args.ionic, args.cutoff, ptm_seq]],
        columns=["pH", "ionic", "cutoff", "fasta"],
        index=[args.name]
    )
